...
- [ ] **10. Installez et configurez l'extension "Trigger Email" avec un compte Gmail d'envoi. / 10. Install and configure the "Trigger Email" extension with a Gmail sending account.**
	- This is a hard prerequisite for the email-first professional application lifecycle. The application may be implemented before this checkbox is complete, but production submission must not claim that verification, approval, rejection, or password-setup email was sent until a real `mail/{messageId}` document is processed successfully by the extension.
