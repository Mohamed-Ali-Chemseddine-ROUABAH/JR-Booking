...
- [ ] **F. Validez le plan des identit̩s de r̩servation avant toute impl̩mentation. / F. Approve the reservation-identity plan before implementation.** Confirm that additional emails are notification contacts only, booking claims require verified email and explicit consent, recurring edits require a selected scope, history sharing is mutual and revocable, and client authentication does not use a short PIN as the sole credential.
