...
The dashboard also exposes read/update/reply queues for `supportTickets` and `dataRequests`, plus a review queue for professional `proClientRecords.platformBanRequest` escalations. Admins can see the subject/type, creator, and current status, move requests between `pending`, `in-progress`, and `completed`, refuse an escalation, or approve it through the trusted `setAccountBanStatus` callable before recording the reviewed CRM status. Review outcomes create bounded requester notifications (`platform-ban-approved` or `platform-ban-rejected`). Authenticated clients and professionals submit bounded requests through `public/js/shared/support-requests.js`; the existing Firestore rules keep these collections private to their creator and admins. Adding an `adminReply` triggers a persisted requester notification: `support-ticket-reply` for support tickets or `data-request-reply` for data requests.

The security section provides two account recovery modes. Preserve mode supplies a target UID or email, generates a Firebase password-reset link server-side, queues it through Trigger Email, and writes an `account-recovery-issued` audit event with `preservedData: true`. Wipe mode additionally requires the exact `EFFACER TOUTES LES DONNEES` confirmation, recursively removes the target's linked bookings/subcollections, profile mirrors/settings, client records, relationships, waitlist entries, notifications, preferences, tokens, and Auth-owned profile data, then preserves the Auth account so the reset link remains usable. The reset URL never reaches the browser; the wipe audit event remains.
