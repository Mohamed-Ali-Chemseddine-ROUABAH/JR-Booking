...
The client detail also derives up to fifteen recent booking and status events. The type and date filters are local to the authorized professional view, and history plus PDF timestamps use the active professional timezone. Status transitions append `{ status, at }` to `bookings.statusHistory`; older bookings without that field still show their current status as a fallback event. `Exporter PDF` opens a print-ready client report containing the identity and the currently filtered history, allowing the browser's PDF destination to save it locally.

When the professional loads booking context, `customRate` replaces the profile rate per unit and `movementSurcharge` replaces the calculated movement-zone fee for that client. The effective values are written only into the authorized booking `paymentContext`; the private CRM record and the professional's standard settings remain unavailable to clients.
