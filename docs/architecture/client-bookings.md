...
- **Purpose:** Renders the client's own bookings sidebar with pending/accepted/rejected filter chips, and marks professional-made pending bookings as awaiting the client's confirmation (`booking.createdBy` set to the professional's UID rather than the client's).
