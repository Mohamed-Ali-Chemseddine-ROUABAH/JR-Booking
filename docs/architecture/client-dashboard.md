...
- **Purpose:** Phase 9 client dashboard: navbar with account/support/notification actions, timezone-aware navigable seven-day read-only schedule, and a bookings sidebar listing the client's own reservations.
...
- **Implemented workflow:** Client payment-context controls, accept/request-change/cancel actions, timezone-aware schedule navigation, locked-professional booking handoff, notification preferences, and the persisted notification center are implemented and Firestore-verified against `firestore.rules`.
