...
- **Rule prerequisite:** Before this slice is implemented, `firestore.rules` must explicitly list the permitted identity/contact/series fields for professional updates and deny those fields to clients except for narrowly defined claim/status operations. New fields must not be added to the current allow-list casually.