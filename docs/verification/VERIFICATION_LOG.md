...
- **Last verified date / Derniere verification:** 2026-09-24.
...
- **Automated regression / Regression automatisee:** full emulator QA now passes cleanly after a clean local Auth/Firestore/Functions/Storage emulator restart: 79/79 tests pass across 37 test files, 72 syntax files, complete architecture coverage, source-size/production-import/secret-exposure/Firebase-config audits, and `git diff --check`. Command: `node tests/run-local-tests.js --with-emulators`.
- **Production deployment / Deploiement production:** deployed successfully on 2026-09-23 — all 53 current functions (explicit `--only` name list), `firestore.rules`, and hosting (85 files) are live. `firebase-functions`/`firebase-admin` were upgraded beforehand to the latest Node-20-compatible versions (7.4.0 / 13.10.0) and re-verified with the clean 79/79 run. Artifact Registry cleanup policy is set for `europe-west9`. Five legacy orphaned functions (`api`, `automatedReminders`, `gcalSyncWorker`, `purgeDeletedProfessionals`, `purgeDeletedTeachers`) remain live but unused; deletion was explicitly declined by the human operator on 2026-09-23 and is a pending decision, not an oversight — re-ask before deleting them. The Cloud Functions Node 20 runtime is deprecated and will be decommissioned 2026-10-30; a Node 22 runtime migration will be required before then and should happen before any `firebase-admin@14.x` upgrade.
- **Human/browser pending / Humain/navigateur en attente:** Waitlist click-through passed against the local emulator. Real Google OAuth and mailbox delivery remain external gates. Populated professional schedule, Today widget, sidebar, printer menu, anonymization toggle, settings menu, and non-Paris timezone rendering were verified in the local browser on 2026-09-14.
- **Next implementation slice / Prochaine tranche:** proceed to remaining external gates (Trigger Email delivery, Google OAuth/calendar consent) and any remaining populated-browser parity checks; no local code, rule, or emulator blocker remains.
