...
## Schedule Parity Contract / Contrat de parite des plannings

Schedule parity is a release requirement for every schedule surface, not only a visual preference. The following production paths must be checked against the approved mockup and the Master Specification together:

| Schedule surface | Required parity scope | Production-specific boundary |
| --- | --- | --- |
| Professional schedule | Day-view switcher, time grid, availability, booking cards, status treatments, date navigation, drag/fixed creation, context actions, sidebar relationship, responsive layout | Show only the active professional's authorized booking details; delegates receive only permitted data/actions. |
| Client schedule | Own bookings, current-week hierarchy, timezone presentation, booking status, acceptance/change-request states, locked professional overlay | Show the client's authorized bookings and anonymous busy ranges only; never expose another client's identity or private booking data. |
| Public profile schedule | Profile hierarchy, day columns, available/occupied treatment, booking entry point, responsive behavior | Use only the safe `publicProfiles` and `busySlots` mirrors; occupied ranges are non-interactive and anonymous. |
| Locked professional schedule | Same schedule geometry and busy-state language as the client/public schedule | Render one selected professional's anonymous `busySlots` overlay at a time and keep favorites/search state separate from schedule authorization. |

Parity means matching the approved visual hierarchy, layout, typography/tokens, controls, interaction states, French labels, status treatment, responsive behavior, timezone display, and privacy-safe state for the same role, viewport, and fixture state. It does not mean copying prototype HTML, inline handlers, or mock data. Any intentional difference must name the governing specification or security reason in the architecture and verification entry.
...
- All applicable schedule surfaces pass the schedule parity contract at desktop and mobile widths, including available, occupied, pending, accepted, and unavailable/locked states where applicable, plus timezone and privacy checks.
