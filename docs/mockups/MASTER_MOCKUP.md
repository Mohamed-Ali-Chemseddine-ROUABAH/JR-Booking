...
**SCHEDULE PARITY REQUIREMENT:** The professional schedule, client schedule, public profile schedule, and locked-professional schedule must retain this mockup's approved hierarchy, grid treatment, controls, status language, responsive behavior, and role-specific visibility through production completion. Production data, authorization, and timezone conversion may differ internally, but the user-visible result must match the approved mockup for the same role, viewport, and state.
