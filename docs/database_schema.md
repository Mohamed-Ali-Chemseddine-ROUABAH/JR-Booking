...
- A client-created booking may set `clientId` only to `request.auth.uid` on create and must omit every other identity, contact, claim, and series field. Direct client updates may not change `contacts`, `clientId`, `serviceRecipientUid`, `seriesId`, `occurrenceIndex`, `claimState`, `claimExpiresAt`, or `claimConflict`.
