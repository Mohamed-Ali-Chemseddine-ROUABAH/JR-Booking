...
export function initializeSidebarFeed(container, { onToggleSidebar, onStatusChange, onEditBooking, onFilterChange, initialFilter = "pending", bookings = [] }) {
    container.innerHTML = `
    ...
    renderBookings(bookingsRoot, bookings, onStatusChange, onEditBooking, activeFilter);
...
function renderBookings(container, bookings, onStatusChange, onEditBooking, activeFilter) {
    ...
    container.innerHTML = visibleBookings.map((booking) => {
        ...
        const actions = booking.status === "pending"
            ? `<div class="sidebar-booking-actions">${editAction}<button class="btn btn-solid" type="button" data-status-action="accepted">${strings.acceptBooking}</button><button class="btn btn-ghost" type="button" data-status-action="rejected">${strings.rejectBooking}</button></div>`
            : booking.status === "accepted"
                ? `<div class="sidebar-booking-actions">${editAction}<button class="btn btn-solid" type="button" data-status-action="done">${strings.completeBooking}</button><button class="btn btn-ghost" type="button" data-status-action="no-show">${strings.noShowBooking}</button></div>`
                : `<div class="sidebar-booking-actions">${editAction}</div>`;
