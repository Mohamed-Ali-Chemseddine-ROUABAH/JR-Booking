...
import { initializeQuietWidgetToggle } from "./today-widget-toggle.mjs";

const strings = UI_STRINGS.proDashboard.today;

export function initializeTodayView(container, bookings = []) {
    const today = new Date();
    const todaysBookings = bookings.filter((booking) => {
        const start = toDate(booking.start);
        return start && sameDay(start, today) && booking.status !== "rejected";
...
export async function initializeNotificationCenter(container, { bookings = [], userId, onSelectBooking } = {}) {
    ...
    if (userId) {
        getDocs(query(collection(getFirestoreDb(), "notifications", userId, "items"), orderBy("createdAt", "desc"), limit(30))).then((snapshot) => {
            ...
            content.innerHTML = renderNotifications(persisted);
...
function renderNotifications(notifications) { return notifications.length ? `<div class="notification-list">${notifications.map((item) => item.notification ? renderPersistedNotification(item) : renderBookingNotification(item)).join("")}</div>` : `<p class="today-empty">${strings.noNotifications}</p>`; }
function renderBookingNotification({ booking, label, action }) { return `<button class="notification-item" type="button" data-notification-booking="${escapeHtml(booking.id)}"><strong>${label}</strong><span>${escapeHtml(getBookingName(booking))} · ${formatDateTime(booking.start)}</span><small>${action}</small></button>`; }
function renderPersistedNotification({ id, notification }) { return `<button class="notification-item${notification.readAt ? " is-read" : ""}" type="button" data-notification-id="${escapeHtml(id)}" data-notification-booking="${escapeHtml(notification.bookingId || "")}"><strong>${escapeHtml(notification.title || strings.notifications)}</strong><span>${escapeHtml(notification.body || "")}</span><small>${notification.readAt ? strings.read : strings.unread}</small></button>`; }
...
function formatTime(value) { const date = toDate(value); return date ? date.toLocaleTimeString("fr-FR", { hour: "2-digit", minute: "2-digit" }) : ""; }
function formatDateTime(value) { const date = toDate(value); return date ? date.toLocaleString("fr-FR", { dateStyle: "short", timeStyle: "short" }) : ""; }
