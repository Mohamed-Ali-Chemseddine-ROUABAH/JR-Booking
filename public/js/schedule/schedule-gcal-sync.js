...
export async function initializeCalendarSync({ user }) {
    ...
    modal.innerHTML = `<section class="glass working-hours-dialog calendar-sync-dialog" aria-labelledby="calendar-sync-title"><div class="working-hours-header"><h2 id="calendar-sync-title">${strings.title}</h2><button class="icon-button modal-close" type="button" data-calendar-close aria-label="${strings.close}" title="${strings.close}"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="m6 6 12 12M18 6 6 18" fill="none" stroke="currentColor" stroke-linecap="round" stroke-width="1.8"/></svg></button></div><p class="calendar-sync-state"><strong>${strings.notConnected}</strong><span>${strings.notConnectedHelp}</span></p><section class="working-hours-section"><h3>${strings.displayTitle}</h3><label class="working-hours-field"><span>${strings.modeLabel}</span><select name="calendarMode"><option value="ghost">${strings.ghostMode}</option><option value="solid">${strings.solidMode}</option></select></label><label class="working-hours-field"><span>${strings.reminderLabel}</span><select name="reminderMinutes"><option value="0">${strings.reminderNone}</option><option value="15">15 minutes</option><option value="30">30 minutes</option><option value="60">1 heure</option></select></label></section><div class="working-hours-actions"><span class="working-hours-feedback" data-calendar-feedback role="status"></span><button class="btn btn-solid" type="button" data-calendar-save>${strings.save}</button><button class="btn btn-ghost" type="button" data-calendar-connect>${strings.connect}</button><button class="btn btn-ghost" type="button" data-calendar-watch>${strings.watch}</button></div></section>`;
