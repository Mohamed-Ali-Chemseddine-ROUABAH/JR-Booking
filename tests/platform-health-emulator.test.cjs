...
const app = admin.initializeApp({ projectId }, `platform-health-${crypto.randomUUID()}`);

test("admin reads aggregate platform health", { timeout: 30000 }, async () => {
    ...
    try {
        ...
        const result = await callFunction("getPlatformHealthSummary", session.idToken, {});
        assert.equal(typeof result.professionals, "number");
        assert.equal(typeof result.clients, "number");
        assert.equal(typeof result.mailFailed, "number");
        await assert.rejects(() => callFunction("getPlatformHealthSummary", clientAuth.idToken, {}), /PERMISSION_DENIED|permission-denied/);
    } finally {
